...
data = np.concatenate(
[np.load('data\\binance_spot\\solfdusd_{}.npz'.format(date))['data'] for date in [20251011]]
)
initial_snapshot = np.load('data\\binance_spot\\solfdusd_20251010_eod.npz')['data']
latency_data = np.concatenate(
[np.load('data\\binance_spot\\solfdusd_{}_latency.npz'.format(date))['data'] for date in [20251011]]
)
...
# 优化
study = optuna.load_study(study_name="name",storage= "sqlite:///mm.db")
